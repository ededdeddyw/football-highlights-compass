...
- 拾ってよい観点の例（当てはまり、かつ確かな情報がある場合のみ・全部入れる必要はない）:
  ...
  - 戦術やプレースタイルの見どころ
- **この試合そのものの結果には触れない**（スコア・勝敗・得点者・PKの行方を書かない）。過去の対戦や移籍などの話題は書いてよい。
- **文章は必ず「これから行われる試合を紹介する」未来・中立の筆致で書く。**「〜となりました」「〜でした」のような、その試合が終わったかのような過去形の実況は書かない。**この試合が延長戦・PK戦にもつれたか（もつれるか）には一切触れない**（延長やPK戦という語自体を、この試合の結末として書かない）。
- **スコアの数字表記（例: 2-1、2対1、3−0）は、過去の試合であっても一切書かない。**「◯対◯」のような数字の勝敗も書かない。対戦成績は「好勝負が続いてきた」「点の取り合いになりやすい」のように質的に述べる。
- **「下した」「制した」「勝利した」「敗れた」「破った」など勝敗を断定する動詞も、過去の試合を含めて使わない。**「互いに譲らない」「見応えのある一戦が多い」のように、勝敗を断定しない書き方にする。
...
- **具体的な年号・在籍年・優勝回数・対戦成績・「通算◯」「◯回目」などの数字は書かない**（確実でも不要。曖昧なら尚更）。「直近の試合で決勝点をアシストした」のような特定試合の個別プレーの断定も、確認できないため書かない。選手は「広く知られた持ち味・役割」の範囲で紹介する。
- 迷ったら、確実に言える一般的な特徴（プレースタイル、大会での大まかな立ち位置、広く知られた選手）に寄せる。
- 落ち着いた紹介の筆致にし、誇張しない。
- **同じ言い回しの使い回しを避ける。**「スタイルの対比」「見応えのある一戦」「中位争い」等のありふれた決まり文句だけで埋めない。その試合ならではの具体（注目選手、会場、順位の文脈、所属クラブの話題など確かに言えること）を優先し、書ける見どころが少なければ無理に増やさず2〜3個でよい。
...
function looksSpoilery(text) {
  ...
  if (/(敗れた|敗れまし|勝利し|勝利を収め|敗北し|完封し|逆転勝ち|下した|下しまし|破った|破りまし|制した|制しまし|白星|黒星|快勝|辛勝|完勝|大勝)/.test(text)) return true;
  // その試合の結末ナレーション（延長・PK戦にもつれた等）＝結果バレ。前フリは未来形で書くべきなので過去完了の実況もNG。
  if (/(PK戦|延長)(に|へ)?(もつれ込|突入し|なりました|の末|で決着)|決着がつか(ず|なかった)|接戦となりました|激闘となり|死闘となり|ドラマ(が待って|の幕切れ)|土壇場で(決|沈|突き放)/.test(text)) return true;
  // 捏造リスクの高い具体数値・断定（生成AIが作りがち）。確実に言えない数字は書かせない。
  if (/[12][0-9]{3}年|通算|[0-9０-９]+回目|[0-9０-９]+度目|[0-9０-９]+連勝|[0-9０-９]+連敗|優勝[0-9０-９]+回|[0-9０-９]+度の優勝|対戦成績|[0-9０-９]+ゴール|[0-9０-９]+得点|得点王/.test(text)) return true;
  return false;
}
