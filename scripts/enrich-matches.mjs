...
記事の要件:
- 日本語で、340〜460文字程度。段落は2〜3個（段落は空行で区切る）。
...
// ネタバレ混入ガード: スコア表記（例 2-1）や勝敗を断定する語があれば不採用にする（安全側）。
function looksSpoilery(text) {
  if (/\d+\s*[-–—]\s*\d+/.test(text)) return true;                 // スコア
  if (/(勝利|敗れ|敗北|完封|逆転勝ち|下した|破った|制した)/.test(text)) return true; // 結果断定
  return false;
}
