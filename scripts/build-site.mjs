...
// 決勝T videoId→ラウンド、およびラウンド→並び用日付。KOはスケジュール日付を持たないため、
// 一覧ソート/カルーセル選定で「グループ戦より新しく・ラウンドが進むほど新しい」順に並ぶよう合成日付を与える（表示用ではなく並び替え専用）。
const KO_ROUND_BY_VID = new Map();
for (const k of Object.keys(WCKO)){ if(!Array.isArray(WCKO[k])) continue; for (const f of WCKO[k]){ if(f && f.videoId) KO_ROUND_BY_VID.set(f.videoId, k); } }
const KO_SORT_DATE = { r32:'2026-06-30', r16:'2026-07-05', qf:'2026-07-10', sf:'2026-07-14', third:'2026-07-18', final:'2026-07-19' };
...
function koBracket(seedMap){
  seedMap = seedMap || {};
  const R32=WCKO.r32||[], R16=WCKO.r16||[], QF=WCKO.qf||[], SF=WCKO.sf||[], FIN=WCKO.final||[], THIRD=WCKO.third||[];
  ...
  const at=(arr,i)=> (i<arr.length ? arr[i] : undefined);
  ...
  // ハイライトへの導線はブラケット下の「対戦カード」一覧に集約（表内に▶は置かず名前の可読性を優先）
  // 1チーム行：シード（組・順位）＋国旗＋名前＋得点（PKは括弧）。得点/勝敗強調は spoiler-cover（既定は伏せる）
  const trow=(m,side,covered)=>{ const name=m?m[side]:null; const w=m?koWin(m):null;
    const win=!!(w&&name&&w===name), lose=!!(w&&name&&w!==name);
    const g=goalsOf(m)[side==='home'?0:1];
    const pkm=String(m&&m.pk||'').match(/^(\d+)-(\d+)$/);
    const pk=pkm?`<span class="br-pk2">(${side==='home'?pkm[1]:pkm[2]})</span>`:'';
    const seedTxt=(name&&seedMap[name])?esc(seedMap[name]):'';
    const nm=covered?`<span class="br-ph">？</span><span class="br-real spoiler-cover">${teamLink(name)}</span>`:teamLink(name);
    const seed=seedTxt?`<span class="br-seed spoiler-cover">${seedTxt}</span>`:'';
    return `<span class="br-team${win?' br-win':''}${lose?' br-lose':''}"><span class="br-info"><span class="br-nm">${nm}</span>${seed}</span>${g!=null?`<span class="br-g spoiler-cover">${esc(g)}${pk}</span>`:''}</span>`; };
  const cell=(m,covered)=>{ const clk=(m&&m.videoId)?` br-clk" data-hl="${esc(m.videoId)}" title="クリックで下のハイライトへ`:'';
    return `<div class="br-cell"><div class="br-match${covered?' br-adv':''}${clk}"><div class="br-rows">${trow(m,'home',covered)}${trow(m,'away',covered)}</div></div></div>`; };
  const col=(title,cells)=>`<div class="br-round"><h4 class="br-h">${title}</h4><div class="br-body">${cells.join('')}</div></div>`;
  // 左右対称・中央収束型。左半分＝R16の前半4枠(＋その供給R32 8試合)、右半分＝後半4枠。
  const L = col('ラウンド32', order.slice(0,8).map(i=>cell(R32[i],false)))
          + col('ベスト16', [0,1,2,3].map(i=>cell(at(R16,i),true)))
          + col('準々決勝', [0,1].map(i=>cell(at(QF,i),true)))
          + col('準決勝', [0].map(i=>cell(at(SF,i),true)));
  const R = col('準決勝', [1].map(i=>cell(at(SF,i),true)))
          + col('準々決勝', [2,3].map(i=>cell(at(QF,i),true)))
          + col('ベスト16', [4,5,6,7].map(i=>cell(at(R16,i),true)))
          + col('ラウンド32', order.slice(8,16).map(i=>cell(R32[i],false)));
  const center = `<div class="br-center">
      <div class="br-trophy" aria-hidden="true">🏆</div>
      <div class="br-h br-h-final">決勝</div>
      ${cell(at(FIN,0),true)}
      <div class="br-third"><div class="br-h">3位決定戦</div>${cell(at(THIRD,0),true)}</div>
    </div>`;
  return `<div class="bracket-wrap"><div class="bracket2">
    <div class="br-side br-left">${L}</div>
    ${center}
    <div class="br-side br-right">${R}</div>
  </div></div>`;
}
...
/* fact card */
.factcard{background:var(--card2);border:1px solid var(--line2);border-radius:14px;padding:6px 18px;margin:24px 0;overflow-x:auto;-webkit-overflow-scrolling:touch}
...
/* 既定（モバイル安全）：通常幅で内部横スクロール。ページ自体は横に伸びない */
.bracket-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch;margin:14px 0 4px;padding:2px 2px 12px;--g:16px;--brline:#d7b13a;scroll-snap-type:x proximity;overscroll-behavior-x:contain}
/* デスクトップのみ：記事幅(860px)を飛び出して画面いっぱいに使う（フルブリード・左右対称マージンで溢れ防止） */
@media(min-width:1000px){.bracket-wrap{width:100vw;margin-left:calc(50% - 50vw);margin-right:calc(50% - 50vw);padding:2px 24px 14px}}
...
.br-info{flex:1 1 auto;min-width:0;display:flex;flex-direction:column;justify-content:center}
.br-nm{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.br-nm .flag{height:12px;width:auto;border-radius:2px;margin-right:5px;vertical-align:-1px}
.br-team a{color:inherit;text-decoration:none}
.br-team a:hover{color:var(--accent2)}
.br-seed{font-size:9px;font-weight:700;color:var(--soft);line-height:1.15;margin-top:1px;letter-spacing:.02em}
.br-g{flex:0 0 auto;margin-left:8px;min-width:12px;text-align:right;font-weight:800;color:var(--ink2);font-variant-numeric:tabular-nums}
.br-g .br-pk2{font-size:.82em;color:var(--muted);margin-left:1px}
html.spoiler-off .br-team.br-win .br-nm{color:#0b1430;font-weight:900}
html.spoiler-off .br-team.br-win .br-g{color:#0b1430}
html.spoiler-off .br-team.br-lose .br-nm,html.spoiler-off .br-team.br-lose .br-g{color:var(--soft);font-weight:600}
...
/* コネクタ（金色）。左半分は右へ、右半分は左へ結線 */
.br-left .br-round:not(:last-child) .br-cell::after{content:"";position:absolute;left:100%;top:50%;width:var(--g);height:2px;background:var(--brline)}
.br-left .br-round:not(:last-child) .br-cell:nth-child(odd)::before{content:"";position:absolute;left:calc(100% + var(--g));top:50%;width:2px;height:100%;background:var(--brline)}
.br-left .br-round:last-child .br-cell::after{content:"";position:absolute;left:100%;top:50%;width:var(--g);height:2px;background:var(--brline)}
.br-right .br-round:not(:first-child) .br-cell::after{content:"";position:absolute;right:100%;top:50%;width:var(--g);height:2px;background:var(--brline)}
.br-right .br-round:not(:first-child) .br-cell:nth-child(odd)::before{content:"";position:absolute;right:calc(100% + var(--g));top:50%;width:2px;height:100%;background:var(--brline)}
.br-right .br-round:first-child .br-cell::after{content:"";position:absolute;right:100%;top:50%;width:var(--g);height:2px;background:var(--brline)}
/* 中央（決勝＋トロフィー＋3位決定戦） */
.br-center{display:flex;flex-direction:column;justify-content:center;align-items:stretch;flex:0 0 138px;min-width:138px;padding:0 6px}
.br-trophy{text-align:center;font-size:30px;line-height:1;margin-bottom:4px}
.br-h-final{color:var(--accent);border-bottom-color:var(--accent2)}
.br-center>.br-cell{flex:0 0 auto}
.br-center .br-match{border-color:var(--accent2);box-shadow:0 2px 10px rgba(39,70,201,.14)}
.br-third{margin-top:24px}
.br-third .br-match{border-color:var(--line2);box-shadow:none}
/* クリックで下のハイライト一覧へジャンプ */
.br-clk{cursor:pointer}
.br-clk:hover{border-color:var(--accent2);box-shadow:0 2px 10px rgba(39,70,201,.18)}
.gx-row.gx-flash{animation:gxFlash 1.6s ease}
@keyframes gxFlash{0%,100%{box-shadow:0 0 0 0 rgba(39,70,201,0)}18%{box-shadow:0 0 0 3px rgba(39,70,201,.5);border-color:var(--accent2)}}
.br-legend{font-size:12px;color:var(--muted);margin:8px 2px 0;line-height:1.7}
...
function buildMatch(m){
  ...
  const head = HEAD({
    title:`${m.ttl} ハイライト動画${lgSeo?'｜'+lgSeo:''} - Football Highlights Compass`,
    ogtitle:`${m.ttl} ハイライト動画${lgSeo?'｜'+lgSeo:''}`, desc, url, ogimg, ogtype:'video.other',
...
function buildPreseasonMatch(mt, season){
  ...
  const desc = `${teamsTxt}のプレシーズン親善試合、結果とハイライト動画をチェック。公式映像のみ掲載、ネタバレ防止表示にも切替可能。`.slice(0,120);
  let videoBlock;
  if (mt.videoId){
    const emb = `<div class="source"><div class="source-head"><span class="tag embed">▶ 公式ハイライト</span><span class="name">YouTube</span><span class="geo">日本で再生可</span></div><div class="embedwrap"><iframe id="ytf_${mt.videoId}" src="https://www.youtube-nocookie.com/embed/${mt.videoId}?enablejsapi=1" loading="lazy" title="${escA(teamsTxt)}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><a class="ytfb" href="https://www.youtube.com/watch?v=${mt.videoId}" target="_blank" rel="noopener"><span class="ytfb-ic">▶</span><span class="ytfb-tx">この試合をYouTubeで見る<small>公式ハイライト</small></span></a></div><a class="ytalt" href="https://www.youtube.com/watch?v=${mt.videoId}" target="_blank" rel="noopener">うまく再生できないときは ▶ YouTubeで見る</a></div>`;
    videoBlock = `<button class="video-veil" type="button" onclick="try{localStorage.setItem('fhc_spoiler','0')}catch(e){};document.documentElement.classList.add('spoiler-off');this.blur();"><span class="vv-ic">🙈▶</span><span class="vv-tx">ハイライト動画はネタバレ防止で隠しています<small>タップで表示（結果が映る場合があります）</small></span></button><div class="spoiler-cover">${emb}</div>`;
...
const TOPSCORERS = {};
try { for(const f of readdirSync('data').filter(n=>/^topscorers-[a-z0-9]+\.json$/.test(n))){ let j; try{ j=JSON.parse(readFileSync(`data/${f}`,'utf8')); }catch{ continue; } if(j && j.code && Array.isArray(j.scorers)) TOPSCORERS[j.code]=j.scorers; } } catch(e){}
...
const GUIDES = [
  ...
  { slug:'avoid-spoilers-how-to-watch', entities:['日本'],
    title:'サッカーの結果を見ずにハイライトを楽しむ方法｜ネタバレ防止ガイド',
    h1:'サッカーの結果・スコアを見ずにハイライトを楽しむ方法',
    dek:'録画やタイムシフトで後から観るとき、SNSや通知でうっかり結果を知ってしまう——を防ぐための実践ガイド。スマホ・DAZN・YouTube・SNSでのネタバレ回避と、当サイトのネタバレ防止機能をまとめました。',
    desc:'サッカーの試合結果やスコアを見ずにハイライトだけ楽しむコツ。SNS・通知・YouTube・DAZNでのネタバレ回避術と、結果を隠したまま観られる当サイトのネタバレ防止機能を解説。',
    body(){
      return `<div class="post-body">
        <p>「試合はあとでゆっくり観たいのに、SNSや通知でスコアを知ってしまった」——サッカーの楽しみを大きく削ぐネタバレは、いくつかのポイントを押さえるだけでかなり防げます。ここでは端末・サービス別の実践的なコツと、結果を隠したままハイライトを観られる当サイトの機能を紹介します。</p>
        <h2 class="lined">① 通知とロック画面を止める</h2>
        <p>最初に効くのが<strong>プッシュ通知の停止</strong>です。観るまでの間だけ、ニュースアプリ・スポーツアプリ・SNSアプリの通知をオフにします。スマホの「集中モード（おやすみモード）」を使い、対象アプリの通知だけを一時的に止めておくと、ロック画面にスコアが出るのも防げます。</p>
        <h2 class="lined">② SNS・検索で結果を踏まない</h2>
        <p>X（旧Twitter）などでは、チーム名・大会名・「速報」「結果」といった語をミュートに設定しておくと、タイムラインに結果が流れにくくなります。検索でも「(チーム名) ハイライト」だけを狙い、「結果」「スコア」を付けない・関連ワードのサジェストを見ないのがコツ。動画サイトはサムネイルや概要にスコアが写ることがあるため、一覧をざっと流し見しないようにします。</p>
        <h2 class="lined">③ 配信・動画サービスの表示に注意</h2>
        <p>見逃し配信やハイライトの一覧では、サムネイルやタイトルに得点が入っていることがあります。目的の試合だけをまっすぐ開き、周辺のおすすめ・関連動画は極力見ないのが安全です。公式の「MATCH RECAP」系はタイトルに結果を出さないものも多く、ネタバレを避けやすい傾向があります。</p>
        <h2 class="lined">④ 当サイトのネタバレ防止機能を使う</h2>
        <p>当サイトは<strong>公式・権利元が公開する映像のみ</strong>を掲載し、<strong>ネタバレ防止</strong>を前提に設計しています。一覧・試合ページでは既定でスコア表記を伏せ、動画ラベルに含まれる得点（例：Team 0-3 Team）も隠しています。スコアは自分のタイミングでのみ確認でき、結果を知らないまま公式ハイライトへ最短でたどり着けます。</p>
        <p style="margin-top:6px">まずはリーグ別の一覧から、観たい試合だけを開いてみてください。</p>
      </div>
      ${LEAGUE_LIST.length?`<h2 class="lined">リーグ別ハイライト（ネタバレ防止つき）</h2><div class="chips">${LEAGUE_LIST.map(h=>`<a href="../league/${h.slug}.html">${esc(h.name)}</a>`).join('')}</div>`:''}
...
function buildLeague(h){
  ...
  // 得点王ランキング（選手別・data/topscorers-<code>.json＝football-data由来）。累計スタッツなので順位表と同レベル（ネタバレ配慮不要）。
  const scList = TOPSCORERS[h.code] || [];
  const scorerBlock = scList.length ? `<h2 class="lined">${esc(h.name)} 得点ランキング</h2>
  <ol class="scorer-list">${scList.slice(0,10).map(s=>{
    const cs=CLUBS[s.team]&&CLUBS[s.team].slug; const tm=cs?`<a href="../club/${cs}.html">${esc(s.team)}</a>`:esc(s.team);
    const a=(s.assists!=null&&s.assists!=='')?`<span class="sc-a">${s.assists}A</span>`:'';
    return `<li><span class="sc-nm">${esc(s.name)}</span><span class="sc-tm">${tm}</span><span class="sc-g">${s.goals}<small>G</small></span>${a}</li>`;
  }).join('')}</ol>
...
let scheduleUrl = '';
{
  ...
  if(all.length){
    ...
    // 今季の得点王（全リーグ横断・各リーグTOP3）。出典 football-data。
    const leagueScorers = [];
    for(const h of LEAGUE_LIST){ const sc=TOPSCORERS[h.code]; if(sc&&sc.length) leagueScorers.push({ name:h.name, slug:h.slug, top:sc.slice(0,3) }); }
    const topScorerBlock = leagueScorers.length ? `<h2 class="lined">今季の得点王（欧州5大リーグ）</h2><div class="rank2">${leagueScorers.map(L=>`<div class="rank-col"><div class="rank-h"><a href="../league/${L.slug}.html">${esc(L.name)}</a></div><ol class="scorer-list">${L.top.map(s=>{ const cs=CLUBS[s.team]&&CLUBS[s.team].slug; const tm=cs?`<a href="../club/${cs}.html">${esc(s.team)}</a>`:esc(s.team); return `<li><span class="sc-nm">${esc(s.name)}</span><span class="sc-tm">${tm}</span><span class="sc-g">${s.goals}<small>G</small></span></li>`; }).join('')}</ol></div>`).join('')}</div><p class="sc-note">※ 出典: football-data.org。各リーグの得点ランキング上位10名は各ハブに掲載。</p>` : '';
    // スケジュールFAQ（GEO：今週の日程・直近ハイライト・得点王の所在をQ&Aで）
    const sfaq = [
    ...
    if(leagueScorers.length) sfaq.push({ q:`今季の欧州5大リーグの得点王は？`, a: leagueScorers.map(L=>`${L.name}は${L.top[0].name}（${L.top[0].goals}ゴール）`).join('、') + '。各リーグの上位10名は各リーグページに掲載しています。出典: football-data.org。' });
    const desc=`欧州5大リーグ（プレミア・ラ・リーガ・セリエA・ブンデス・リーグアン）の今週の試合日程と今季の得点王を日本時間で一覧。対戦カード・キックオフ時刻・得点ランキングをまとめ、試合後は公式ハイライトへ。`.slice(0,120);
...
let WCHUB_HTML = '';   // トップ左ナビ「W杯26ハブ」（決勝T＋全組リンク）。index.html の WCHUB マーカーへ注入
{
  ...
  // ===== 決勝トーナメント（日程＋各組の進出国。対戦カード/ハイライトはグループ最終確定後に順次反映） =====
  {
    ...
    // シード（組・順位）マップ：各組の順位表から 国名→「A1位」等。ブラケットの各国に付す
    const seedMap = {};
    for(const L of letters){ const t=tableFor(L); t.ranked.slice(0,3).forEach((nm,i)=>{ if(nm && !seedMap[nm]) seedMap[nm] = `${L}${i+1}位`; }); }
    ...
    const koRow = (m, tag) => { const h=m.home,a=m.away;
      ...
      return `<div class="gx-row"${m.videoId?` id="ko-${esc(m.videoId)}"`:''}><span class="gx-rd">${tag}</span><span class="gx-tm">${hl} ${vs} ${al}</span><span class="gx-meta"></span>${right}</div>`;
    };
  ...
  <p class="br-hint">← 横スクロールで決勝まで表示 →</p>
  ${koBracket(seedMap)}
  <p class="br-legend">名前の下は組・順位（例：A1位）／「？」「未定」＝結果確定前 ／ <b>表の試合カードをクリックすると下の「対戦カード」一覧の該当試合へ移動します</b>。R16以降の勝ち上がりは既定で伏せています（ネタバレ防止）。</p>
  <script>(function(){var w=document.querySelector('.bracket2');if(!w)return;w.addEventListener('click',function(e){if(e.target.closest('a'))return;var m=e.target.closest('.br-match[data-hl]');if(!m)return;var t=document.getElementById('ko-'+m.getAttribute('data-hl'));if(!t)return;t.scrollIntoView({behavior:'smooth',block:'center'});t.classList.remove('gx-flash');void t.offsetWidth;t.classList.add('gx-flash');});}());</script>
