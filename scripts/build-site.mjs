...
function koBracket(seedMap){
  ...
  const cell=(m,covered)=>`<div class="br-cell"><div class="br-match${covered?' br-adv':''}"><div class="br-rows">${trow(m,'home',covered)}${trow(m,'away',covered)}</div></div></div>`;
...
let WCHUB_HTML = '';   // トップ左ナビ「W杯26ハブ」（決勝T＋全組リンク）。index.html の WCHUB マーカーへ注入
{
  ...
  ${koBracket(seedMap)}
  <p class="br-legend">名前の下は組・順位（例：A1位）／「？」「未定」＝結果確定前 ／ 表は横にスクロールできます。各試合の公式ハイライトは下の「対戦カード」一覧から。R16以降の勝ち上がりは既定で伏せています（ネタバレ防止）。</p>
