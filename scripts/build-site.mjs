...
let WCHUB_HTML = '';   // トップ左ナビ「W杯26ハブ」（決勝T＋全組リンク）。index.html の WCHUB マーカーへ注入
{
  ...
  // ===== 決勝トーナメント（日程＋各組の進出国。対戦カード/ハイライトはグループ最終確定後に順次反映） =====
  {
    ...
    // ラウンド32 対戦カード（data/wc-knockout.json）。結果はネタバレ防止で既定隠し（spoiler-cover）
    const r32 = (WCKO.r32||[]);
    const r32rows = r32.map(m=>{ const h=m.home,a=m.away;
      ...
      return `<div class="gx-row"><span class="gx-rd">R32</span><span class="gx-tm">${hl} ${vs} ${al}</span><span class="gx-meta"></span>${right}</div>`;
    }).join('');
    const r32Section = r32.length ? `<h2 class="lined">ラウンド32 対戦カード</h2><p class="note-sm">対戦カードはSI.com・Sky Sportsで相互確認（2026-06-28）。スコアは試合消化後に反映し、既定は非表示（下のボタンで表示）。</p><div class="gx-list">${r32rows}</div>` : '';
